...
路由的结构体的设计是为了将 Trie 树应用到路由中。不用Trie的普通路由即URL作为key，request method + URL 对应的处理函数作为value。
```go
type router struct {
...
// node结构体的patten成员，只有当该node已经是最底层的时候，才会赋值为完整的URL，否则为空
// 查找子节点，若找不到则新建子节点（赋值两个变量：父节点part和isWild是否检测到冒号和星号动态路由标志）并将子节点接入结构体的子节点成员变量中
// 递归对子节点做相同（本身函数）的操作
...
```go
// 返回值*node是找到最底层匹配的节点
// 返回值params的数据类型是map[string]string，key/value是获取动态路由（冒号，星号）匹配的参数和值
...
* 通配符*，但只支持一个*，因为*通配符就是匹配当前和后面的所有，只需要匹配1个的情况
