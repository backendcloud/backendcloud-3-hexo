...
![](/iimages/golang-std_heap/2022-12-12-14-39-23.png)

有了上面的堆特性的概念，理解down方法就不难了。down方法的三个入参分别是 heap， 需要下层down的元素下标， heap的长度。返回值是是否对元素做了下沉操作。for循环是遍历完该元素的每一层，直到最后一层（一旦检测到不需要下层了，就是该元素比下一层的两个元素都要小，跳出for循环）。
...
然后和父元素比较，若父元素比较小的子元素还要小，则该父元素不需要再下层了，跳出for循环，否则需要下层至下一层较小的元素的位置，继续for循环进入再下一层比较。

init方法和pop方法用到了down方法。init就是从倒数第二层开始遍历所有元素直到顶层对每个元素调用下层方法。pop方法就是弹出顶层的最小元素，然后调用down方法重新排序填补顶层元素的位置。
...
Remove方法和Fix方法都涉及到了对index为i的元素先下层，若没有下层则上浮的操作。Remove是因为删除了一个元素需要填补空缺，Fix因为该元素的value被改了，需要重新排序。
